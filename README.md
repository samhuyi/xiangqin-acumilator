...
七个套件当前全绿（art / ui / click / cloud / encounter / smoke / edge；`ui-smoke.js` 共 480+ 条断言）。其中 `ui-smoke.js` 额外回归了「设定页四步单选（点选 / 取消 / 回退不留高亮）」「点头像看资料」「**近期变化的入口在主角头像上**（属性条不再是可点区域、提示文案指向头像、点击命中区进入变化页）」「**好感度条挂在对象头像下方**（数据里 `stat_defs` 的 `slot=partnerHeader` 决定位置；没有对象时整块不出现；标签左对齐在对象那一列、数值右对齐到列右边缘）」「**属性条涨跌动效**（条长起点=旧值、终点=新值，涨色段正好盖住本次涨幅，跌色段紧跟实心条尾部并在动画结束时淡尽不留下虚长，数值滚动到正确中间帧）」「标题页按钮起点 ≥ 画布 50% 且无白色蒙层」「头像从数据库读取（改库里的 avatar 值即刻生效）」「相亲机会结果页的花销框 + 信封卡五项背景框」「近期经历页的事件 / 选择 / 结果」「微信闲聊（入口位置、气泡、**与事件同一套结算**（完整 `fx`，可含 `fx.zero`）、结算按钮写明进入下一天且正好推进 `ACTION_DAYS.chat` 天）」「**对方主动发微信**（顶部标出「主动发来」、那一次没有「返回」、聊完换到新的一天即跳过当天行动选择、计数 +1）」「**灵魂拷问 / 暧昧事件**（按人格出题、踩雷掉 ≥20 好感并提示答错、答对涨 ≥15 好感并提示答对）」「**必须当面约会**（入口不消失而是换副标题、点进去只显示「我们还是多当面接触吧」、安排约会后解锁并清零计数）」「事件确认后原页展示结果（不跳页、头像/背景保留、选项区不再残留可点项）」「倒计时只在「单身且手上没人」时扣减：接触中暂停 / 已有对象待初遇也暂停 / 到点但接触中不判负 / 卡片分别提示三种暂停原因」「**属性结算不再随机浮动**（同一组效果两次结果一致、数值等于事件写死的值；`FX_ROLL_*` 常量已从数据库删除）」「**第一印象不再随机**（同一个人两次见面结果一致）」「**相亲对象来自数据库**（花名册分男女、字段齐全、职业可解析；抽 40 次全落在花名册；同一局优先抽没见过的，12 位异性全覆盖不重复；`s.met` 记账）」「**相亲图鉴**（解锁记录 / 阶段 / 好感峰值 / 未遇见不入账 / 分栏切换 / 黑框问号占位 / 资料页 / 主界面入口）」「**告白结算页带头像与背景**（命中场景、头部高度 200）」「**行动菜单只有两个方向入口**（两张单选卡片 + 底部「确认」才进二级页、微信闲聊等已收进二级页）」「**两个二级页都有页头背景图**（页标题压在页头里 y≈62 < 头部 156、标题按「主角性别」取到不同的图、二级页不重复铺状态区）」「**结局页头部插画**（14 个结局都有 `art`、只用到 3 种画面、页头画出小标签、结局标题排在插画下方、题目类型/性别各两张图都在注册表里）」「**微信题库为空时的降级**（入口仍在、点进去给说明页而不是空白、可返回、并打出一条指向「重新导入 chats.json」的告警）」「**首页按钮新样式**（没有整屏白色蒙层 / 每个按钮都有渐变底色 / 底色 70% 不透明 / 边缘渐隐到全透明 / 文字水平居中 / 字号 ≥17 比通用按钮大一号 / 大一号后仍全部落在屏幕内）」「**页面过渡动效**（换页时指纹改变才触发、中途位移与透明度单调收敛、结束后回到终态不留残影、同一页重复绘制不重播、测试模式下恒为终态）」「**提示条不被每帧重绘吃掉**（第 30 天结算入队后连续多次重绘仍在，TTL 内可读）」「**微信聊够次数就要求当面见一面**（玩家自己点开的也算，入口副标题显示 2/2，下一段强制固定剧情并锁定）」「**交往风格是叠层不是新页**（叠层打开时下层页面不再登记可点区域、面板内可滚动、返回/确认两个按钮、关闭后整页与背景不跳动）」「**极端后果用 fx.zero 表达**（0 个超大负数、归零一律停在保底 1 点、不会当场触发结局、zero 只声明七项基础属性）」「**关系预警常驻主界面关系卡**（不用靠一闪而过的提示条；把「哪一项出问题」原文写出来；预警解除后提示条消失）」，以及**布局回归**：主界面（单身 / **待赴约待见面** / 恋爱中·好感 480 三位数最宽态 / **关系预警挂着** / **预警已解除**）、**行动菜单与两个二级页**、**约会档位页（带背景头部）与交往风格叠层**、事件页（选项态 / 结果态）、对方资料、近期变化、近期经历、微信闲聊（含灵魂拷问选项态 / 答错提示 / 必须当面见面提示 / **题库为空**）、**相亲图鉴（两栏）与图鉴资料页**、相亲结果页、**告白结算页（带头部）**、**14 个结局 + 双人头像版结局页**逐页渲染，用 CJK 感知的字宽模型检测「文字互相重叠」，并断言信封卡内容不溢出卡片底部；`art-smoke.js` 额外回归了「赴约初遇的背景兜底」「有相亲机会时对方头像真的画出来」「信封卡上的对象头像被 drawImage」「**10 张按需页头背景全部可解码**（行动页按性别取到不同图、结局图分男女、没有美术类型时返回 null 走纯色兜底）」，以及**真机核心回归**「分包下载慢于重试预算时图仍要全部加载上」（用 `subpackageDelay` 模拟慢分包，并让分包内的文件在下载完成前真的取不到 —— 这条能复现「除了主界面图全都加载失败」）。
...
插画路径 `assets/images/end/end_<art>_<性别>.jpg`，其中 `<art>` 来自结局记录的 **`art` 字段**，14 个结局只对应 3 种画面：
...
> **时间上限（`maxDays`）的兜底结局按关系状态分流**（`engine.timeoutEndingId`）：
> 单身 → `timeout`（「你既没有走进婚姻」只对单身成立）、有对象但未婚 → `stalled`（停在原地）、
> 已婚 → `married_stall`（婚姻里的将就）。否则玩「城市立足」这类跟婚恋无关的目标时，
> 求婚成功反而会被判成「没走进婚姻」。
