...
module.exports = [
   ...
   * 极端事件（x_ 前缀）
   * ---------------------------------------------------------
   * 这批和上面的日常事件不是一个量级：后果是断崖式的，
   * 所以一律用 fx.zero 表达「某项属性直接见底」，而不是写一个大负数。
   *
   * 为什么用 zero 而不是 money: -999999：
   *   · 结算面板会显示成「存款 -99 万」这种没有信息量的数字；
   *   · 更关键的是 —— 写大负数会真的把属性打到 0，而存款 / 健康 /
   *     事业 / 情绪任意一项归 0，checkEnd 都会立刻判负（破产 / 身体
   *     亮红灯 / 事业崩塌 / 撑不住了）。一条随机事件秒杀玩家，
   *     等于这一局被随机数吃掉，没有反打余地。
   *   · zero 的语义是「洗空」，引擎会落到 FX_ZERO_FLOOR（默认 1 点）：
   *     见底的体感保留，命还留着。
   *
   * 所以：写 zero 的那个选项，就不要再写同一属性的大负数了 ——
   *     加法会先把属性打到 0，zero 再去兜底，等于白兜（引擎已经处理了
   *     这个顺序，但数据上仍然应该只表达一次「归零」）。
   *
   * 每个选项最多写 3 项影响（tools/rebalance-fx.js 会按幅度截前 3），
   * 数值仍按「作者意图」写小一档，复算时统一 ×2。
   * ========================================================= */
  {
    id: 'x_single_hunmei', phase: 'single', weight: 3,
    text: [
      '婚介所推来的「优质会员」，照片修得挑不出毛病，条件也好得不像真的。',
      '第二次见面，她说表哥的店在众筹，问你要不要一起投一点。'
    ],
    options: [
      { label: '把存款都交给她打理', fx: { zero: ['money'], mood: -12 }, result: '转账那天她笑得很甜，还说下次带你去见她妈。第三天，她的头像换成了一只猫，手机号成了空号。' },
      { label: '只拿两万试水', fx: { money: -10000, mood: -6, career: -2 }, result: '钱没回来，人也没再出现。你回婚介所问，前台翻了半天系统说：「这位会员的资料上周就注销了。」' },
      { label: '先要一份那家店的工商信息', fx: { mood: 6, career: 3, money: -150 }, result: '她把话题岔开了两次。第三次你追问，她说「算了，我们不合适」。你回到车里，坐了很久。' }
    ]
  },
  {
    id: 'x_talking_privacy', phase: 'talking', weight: 3,
    text: [
      '她拿你手机点外卖，回来的时候手指停在屏幕上没动。',
      '你走过去看到，那是你和前任的最后一段对话——还有一份你从来没跟任何人提过的检查单。'
    ],
    options: [
      { label: '把这两件事从头说一遍', fx: { zero: ['mood'], affection: 6, family: -4 }, result: '你说了很久，她一直没打断。说完她说：「你早点告诉我就好了。」那天你们没再聊别的。' },
      { label: '把手机抽回来：「你翻我东西？」', fx: { affection: -18, mood: -8, family: -3 }, result: '她愣了一下，说「对不起」，然后回了房间。门关得很轻，你反而更难受。' },
      { label: '说那是同事的手机', fx: { affection: -10, mood: -5, career: 1 }, result: '她笑了一下，说「好」。之后每次你手机响，她都会有意无意地看你一眼。' }
    ]
  },
  {
    id: 'x_any_guarantee', phase: 'any', weight: 3,
    text: [
      '短信弹出来：你名下的一笔借款已逾期九十天，本息合计四十七万。',
      '你翻遍手机才想起来——三年前，你替一个朋友在担保人那一栏签过字。'
    ],
    options: [
      { label: '把存款填进去，先把它清掉', fx: { zero: ['money'], mood: -14 }, result: '你在银行柜台坐了一个多小时。朋友的电话打不通，微信头像旁边写着「该用户不存在」。' },
      { label: '不管，让他自己上征信', fx: { mood: -6, career: -8, family: -6 }, result: '催收电话开始打到你公司。同事都听见了，没人问，也没人替你说话。' },
      { label: '找律师，走诉讼', fx: { money: -8000, mood: -5, career: 4 }, result: '律师说担保责任跑不掉，但程序上有得打。你签了委托书，第一次觉得「懂一点法」能这么值钱。' }
    ]
  },
  {
    id: 'x_dating_control', phase: 'dating', stage: 'dating', weight: 3,
    text: [
      '她把手机递过来：一个共享位置的 App，界面里两颗头像挨在一起。',
      '「这样比较有安全感。」她说。你翻到自己那一条，发现定位记录从三个月前就开始了。'
    ],
    options: [
      { label: '装上，让她安心', fx: { affection: 8, mood: -10, family: -5 }, result: '一开始只是位置，后来是相册，再后来是通话记录。你删掉过一次，她哭了整晚。' },
      { label: '「这东西我不装」', fx: { affection: -14, mood: -4, career: 2 }, result: '她把手机收回去，说「那你随便」。接下来三天，她回消息都很慢。' },
      { label: '「三个月前的记录，你哪来的？」', fx: { zero: ['mood'], affection: -16 }, result: '她一句一句往下说，连你那天几点几分进的电梯都记得。你坐在那儿，越听越冷。' }
    ]
  },
  {
    id: 'x_dating_married_secret', phase: 'dating', stage: 'dating', weight: 2,
    text: [
      '一个陌生号码加你微信，发来一张全家福。',
      '她就站在照片中间，怀里抱着一个三四岁的孩子，旁边是结婚证的红本。'
    ],
    options: [
      { label: '把照片转给她，只问一句「解释一下」', fx: { zero: ['mood'], affection: -40 }, result: '她回了很长一段，说「分居两年了，手续一直没办」。你从头到尾只回了两个字：知道了。' },
      { label: '打给那个号码，问到底是谁', fx: { mood: -12, health: -4, family: -6 }, result: '接电话的是个男人的声音。他没骂你，只说了一句：「兄弟，你是第三个了。」' },
      { label: '装作没收到，继续过', fx: { affection: 8, mood: -14, family: -8 }, result: '你把那条消息删了。之后每次她说「以后」，你都会想起那张照片。' }
    ]
  },
  {
    id: 'x_any_checkup', phase: 'any', weight: 3,
    text: [
      '体检报告出来了，医生用笔在一行数字下面划了两道。',
      '「这个要复查，尽快。」他把「尽快」两个字说得很慢。'
    ],
    options: [
      { label: '当场请假，当天就复查、住院', fx: { money: -30000, health: 7, mood: -8 }, result: '结果是早期，医生说「你来得算及时」。病床上你盯着天花板，第一次觉得钱花在这儿不心疼。' },
      { label: '「等项目忙完这阵子」', fx: { zero: ['health'], mood: -12 }, result: '你把它塞进抽屉最底下。三个月后，你在地铁上蹲下去起不来，被好心人送进了急诊。' },
      { label: '先买一份重疾险再说', fx: { money: -10000, mood: -5, career: 2 }, result: '保险顾问一条条念免责条款。你听得认真，却一句都没听进去。' }
    ]
  }
];
